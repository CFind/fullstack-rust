        ...
        (Ok(id), Ok(mut client)) => 
            match client.query_one("SELECT * FROM users WHERE id = $1", &[&id]) {
                Ok(row) => {
                    ...
                    (OK_RESPONSE.to_string(), serde_json::to_string_pretty(&user).expect("SHOULD return the user"))
                }
...
fn handle_get_all_request(_request: &str) -> (String, String) {
    match Client::connect(DB_URL, NoTls) {
        Ok(mut client) => {
            ...
            (OK_RESPONSE.to_string(), serde_json::to_string_pretty(&users).expect("SHOULD return the users"))
        }
    ...
    match (
        ...
        (Ok(id), Ok(user), Ok(mut client)) => {
            client.execute(
                ...
                &[&user.name, &user.email, &id]).expect("SHOULD update exactly one row.");
...
fn handle_delete_request(request: &str) -> (String, String) {
    match (get_id(&request).parse::<i32>(), Client::connect(DB_URL, NoTls)) {
        (Ok(id), Ok(mut client)) => {
                let rows_altered: u64 = client.execute(
