...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
    ...
    const buttonColors: { [key: string]: string } = {
        entech: 'bg-green-500, hover:bg-green-700',
    };

    const bgColor = backgroundColors[backendName as keyof typeof backgroundColors] || 'bg-gray-200';
    const btnColor = buttonColors[backendName as keyof typeof buttonColors] || 'bg-gray-500 hover:bg-gray-700';
...
return (
    ...
    <div className={`user-interface ${bgColor} ${backendName} w-full max-w-md p-4 my-4 rounded shadow`}>
        ...
        <Image src={`/${backendName}logo.svg`} alt={`${backendName} Logo`} className="w-20 h-20 mb-6 mx-auto" width={20} height={20}/>
